...
def get_system_info():
    ...
    if (info['make'] or '').upper().startswith('LENOVO') and _product_version:
        ...
        _ver_is_friendly = (
            _product_version and (
                ' ' in _product_version or
                any(_product_version.upper().startswith(k.upper())
                    for k in _friendly_kws)
            )
        )
        if _name_is_sku and _ver_is_friendly:
            info['model'] = _product_version
        else:
            info['model'] = _product_name or _product_version or 'Unknown'
    else:
        info['model'] = _product_name or 'Unknown'
