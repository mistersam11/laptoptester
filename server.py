...
# Log file lives in the directory the server is run from
LOG_FILE      = Path.cwd() / "LaptopSyncServer.log"


def setup_logging():
    """
    Configure logging to write to both console and a file in the current
    working directory. The file is created/overwritten each time the server
    starts.
    """
...
def setup_directory():
    ...
    if not TEMPLATE_FILE.exists():
        logger.error("Template file not found. Expected at %s", TEMPLATE_FILE)
        print(f"\n  ERROR: Template file not found.")
        print(f"  Please place 'Laptop_Templates.xlsx' in:")
        print(f"  {DOCUMENTS_DIR}\n")
        sys.exit(1)
    logger.info("Template : %s", TEMPLATE_FILE)
    logger.info("Log file : %s", DAILY_FILE)
    print(f"  Template : {TEMPLATE_FILE}")
    print(f"  Log file : {DAILY_FILE}")

def find_desktop_header_row(ws):
    """Return the row index of the desktop header row (second occurrence of the CSAD header), or None."""
    for row in ws.iter_rows(min_row=2):
        if row[0].value == "Device Info - CSAD Item ID":
            return row[0].row
    return None
...
def initialize_daily_log():
    ...
    # Copy laptop header row (row 1)
    for col_idx, cell in enumerate(template_ws[1], start=1):
        new_ws.cell(row=1, column=col_idx, value=cell.value)

    # Find and copy desktop header row from template (row 2 = empty gap, row 3 = desktop header)
    desktop_header_template_row = find_desktop_header_row(template_ws)
    if desktop_header_template_row:
        for col_idx, cell in enumerate(template_ws[desktop_header_template_row], start=1):
            new_ws.cell(row=3, column=col_idx, value=cell.value)

    new_wb.save(DAILY_FILE)
    logger.info("Created new daily log: %s", DAILY_FILE)
...
def append_to_log(data):
    ...
    # Determine whether this is a desktop or laptop based on category
    category = str(new_row[3]).lower() if len(new_row) > 3 and new_row[3] else ""
    is_desktop = "desktop" in category

    # Find the desktop header row in the daily log
    desktop_header_idx = find_desktop_header_row(daily_ws)

    if is_desktop:
        if desktop_header_idx:
            # Find the last row with data at or after the desktop header
            last_desktop_row = desktop_header_idx
            for row in daily_ws.iter_rows(min_row=desktop_header_idx + 1):
                if any(cell.value for cell in row):
                    last_desktop_row = row[0].row
            # Write new desktop row directly after last desktop row
            insert_idx = last_desktop_row + 1
            for col_idx, value in enumerate(new_row, start=1):
                daily_ws.cell(row=insert_idx, column=col_idx, value=value)
            row_idx = insert_idx
        else:
            # No desktop header found — fall back to appending
            daily_ws.append(new_row)
            row_idx = daily_ws.max_row
    else:
        if desktop_header_idx:
            # Insert the laptop row just before the empty gap row (desktop_header_idx - 1).
            # insert_rows shifts the gap + desktop section down by one, preserving the gap.
            insert_idx = desktop_header_idx - 1
            daily_ws.insert_rows(insert_idx)
            for col_idx, value in enumerate(new_row, start=1):
                daily_ws.cell(row=insert_idx, column=col_idx, value=value)
            row_idx = insert_idx
        else:
...
def log_laptop():
    ...
    try:
        ...
        logger.info("Logged model '%s' (CSAD: %s)", model, data.get("csad_value", "none"))
        ...
        logger.warning("Model not found in template: %s", e)
        return jsonify({"status": "error", "message": str(e)}), 404

    except Exception as e:
        logger.exception("Error while logging laptop data")
        print(f"  ERROR: {e}")
